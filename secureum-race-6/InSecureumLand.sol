...
contract InSecureumLand is ERC721Enumerable, Ownable, ReentrancyGuard, VRFConsumerBase {
    ...
    // events
    event LandPublicSaleStart(uint256 indexed _saleDuration, uint256 indexed _saleStartTime);
    event LandPublicSaleStop(uint256 indexed _currentPrice, uint256 indexed _timeElapsed);
    ...
    ) external whenPublicSaleActive nonReentrant {
        require(numLands > 0, 'Must mint at least one beta');
        require(
            currentNumLandsMintedPublicSale + numLands <= MAX_PUBLIC_SALE_AMOUNT,
    ...
    function stopContributorsClaimPeriod() external onlyOperator whenContributorsClaimActive {
        contributorsClaimActive = false;
        emit ContributorsClaimStop(block.timestamp);
    }

    function contributorsClaimLand(
        uint256 amount,
        address recipient
    ) external onlyContributors(msg.sender) whenContributorsClaimActive {
    ...
    function claimUnclaimedAndUnsoldLandsWithAmount(
        ...
        if (betaClaimedAmount < MAX_BETA_NFT_AMOUNT) {
            ...
            uint256 toMint = leftToBeMinted < maxAmount ? leftToBeMinted : maxAmount; //take the min
        ...
        if (mintIndexPublicSaleAndContributors < MAX_LANDS) {
            ...
            for (
                ;
                mintIndexPublicSaleAndContributors < MAX_LANDS && totalMinted < toMint;
    ...
    function mintFutureLandsWithAmount(
        address recipient,
        uint256 maxAmount
    ) public onlyFutureMinter {
        require(maxAmount <= MAX_MINT_PER_BLOCK, 'maxAmount cannot exceed MAX_MINT_PER_BLOCK');
        require(
        ...
        for (
            uint256 claimed;
            claimed < maxAmount && futureLandsNftIdCurrent < MAX_LANDS_WITH_FUTURE;
            ++claimed
        ) {
